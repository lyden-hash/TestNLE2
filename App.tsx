  ...
  const SidebarContent = (
    <nav className="space-y-8">
      <div className="space-y-1">
        <label className="text-[10px] font-bold text-slate-500 uppercase tracking-widest px-3 mb-2 block">Enterprise Suite</label>
        ...
        <div className="p-8 animate-fade-in overflow-y-auto h-full">
          <header className="flex justify-between items-end mb-10">
            <div>
              ...
              <p className="text-slate-400 mt-1">Review and manage active project bids.</p>
            </div>
          </header>
            ...
            <table className="w-full text-left">
              <thead>
                <tr className="bg-slate-800/40 border-b border-slate-800">
                  <th className="px-8 py-5 text-[10px] font-bold text-slate-500 uppercase tracking-widest">Project</th>
                  ...
                  <th className="px-8 py-5 text-[10px] font-bold text-slate-500 uppercase tracking-widest text-right">Value</th>
                  <th className="px-8 py-5 text-[10px] font-bold text-slate-500 uppercase tracking-widest text-center">Action</th>
                </tr>
              </thead>
                ...
                {estimates.map(est => (
                  <tr key={est.id} className="hover:bg-indigo-500/5 transition-colors group cursor-pointer" onClick={() => { setSelectedEstId(est.id); setView('builder'); resetAISidebars(); }}>
                    <td className="px-8 py-6">
                      ...
                      <div className="text-[10px] text-slate-500 font-bold uppercase tracking-tight mt-1">{est.location}</div>
                    </td>
                    ...
                    <td className="px-8 py-6 text-right font-mono font-bold text-white">${est.total.toLocaleString()}</td>
                    <td className="px-8 py-6 text-center">
                      <button className="w-8 h-8 rounded-full bg-slate-800 text-slate-500 hover:bg-indigo-600 hover:text-white transition-all"><i className="fas fa-chevron-right text-xs"></i></button>
                    </td>
                  </tr>
                ))}
              </tbody>
      ...
      {view === 'customers' && (
        <div className="p-8 animate-fade-in overflow-y-auto h-full">
           <header className="mb-10">
            <h1 className="text-3xl font-bold text-white tracking-tight">Client Directory</h1>
            <p className="text-slate-400 mt-1">Manage relationships with owners, developers, and general contractors.</p>
          </header>
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            {MOCK_CUSTOMERS.map(cust => (
              <div key={cust.id} className="bg-slate-900/50 border border-slate-800 rounded-3xl p-8 backdrop-blur-sm hover:border-indigo-500/50 transition-all group shadow-xl">
                <div className="flex justify-between items-start mb-6">
                  <div className="w-14 h-14 bg-slate-800 rounded-2xl flex items-center justify-center text-indigo-400 group-hover:bg-indigo-600 group-hover:text-white transition-all shadow-inner">
                    <i className="fas fa-building text-2xl"></i>
                  </div>
                  <span className={`px-3 py-1 rounded-full text-[9px] font-bold uppercase tracking-widest border ${cust.type === 'GC' ? 'bg-blue-500/10 text-blue-400 border-blue-500/20' : 'bg-purple-500/10 text-purple-400 border-purple-500/20'}`}>
                    {cust.type}
                  </span>
                </div>
                ...
                <div className="space-y-3 mt-6">
                  <div className="flex items-center gap-3 text-slate-400 text-xs">
                    <i className="fas fa-phone w-4 text-center text-slate-600"></i>
            ...
            <div className="flex items-center gap-6">
              ...
              <div>
                <input 
                  ...
                  className="bg-transparent text-2xl font-black text-white outline-none focus:text-indigo-400 transition-colors w-[400px]"
                />
              </div>
            </div>
          ...
          <div className="flex-1 flex overflow-hidden">
            <div className="flex-1 overflow-y-auto p-12 space-y-12 custom-scrollbar bg-slate-950/50">
               <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
                 <div className="space-y-3">
                    <label className="text-[10px] font-bold text-slate-500 uppercase tracking-widest px-1">Customer</label>
                    <select 
                 ...
                 <div className="space-y-3">
                    <label className="text-[10px] font-bold text-slate-500 uppercase tracking-widest px-1">Market Location</label>
                    <input 
                      type="text" 
               ...
               <div className="space-y-6">
                 ...
                 <div className="bg-slate-900/50 border border-slate-800 rounded-3xl overflow-hidden backdrop-blur-sm shadow-xl">
                    <table className="w-full text-left">
                      <thead>
                        <tr className="bg-slate-800/40 border-b border-slate-800">
                          <th className="px-8 py-5 text-[10px] font-bold text-slate-500 uppercase tracking-widest">Scope</th>
                          <th className="px-8 py-5 text-[10px] font-bold text-slate-500 uppercase tracking-widest w-28">Qty</th>
                          <th className="px-8 py-5 text-[10px] font-bold text-slate-500 uppercase tracking-widest w-44">Rate</th>
                          <th className="px-8 py-5 text-[10px] font-bold text-slate-500 uppercase tracking-widest w-44 text-right">Ext. Total</th>
                        ...
                        {activeEstimate.lineItems.map((item, idx) => (
                          <tr key={item.id} className="group hover:bg-white/5 transition-colors">
                            <td className="px-8 py-5">
                              ...
                              <input className="bg-transparent text-slate-500 text-[10px] outline-none w-full mt-1" value={item.description} onChange={(e) => {
                                const newItems = [...activeEstimate.lineItems];
                            ...
                            <td className="px-8 py-5">
                              <input type="number" className="bg-slate-800/50 px-3 py-1.5 rounded-lg outline-none w-full font-mono text-xs border border-slate-700 focus:border-indigo-500" value={item.qty} onChange={(e) => {
                            ...
                            <td className="px-8 py-5">
                              <input type="number" className="bg-slate-800/50 px-3 py-1.5 rounded-lg outline-none w-full font-mono text-xs border border-slate-700 focus:border-indigo-500" value={item.rate} onChange={(e) => {
                                const val = Number(e.target.value);
                                const newItems = [...activeEstimate.lineItems];
                            ...
                            <td className="px-8 py-5 text-right font-mono font-black text-indigo-400">${item.amount.toLocaleString()}</td>
                  ...
                  <div className="bg-slate-900 border border-slate-800 rounded-[40px] p-10 space-y-6">
                    ...
                    <div className="grid grid-cols-2 gap-6">
                      <div className="space-y-2">
                        <label className="text-[9px] font-bold text-slate-500 uppercase tracking-widest px-1">Profit Margin (%)</label>
                        <input 
                          type="number"
                          className="w-full bg-slate-950 border border-slate-800 rounded-2xl px-4 py-3 text-sm text-indigo-400 font-mono outline-none focus:border-indigo-500"
                          value={activeEstimate.margin || 0}
                          onChange={(e) => handleUpdateEstimate({ margin: Number(e.target.value) })}
                        />
                      </div>
                      <div className="space-y-2">
                        <label className="text-[9px] font-bold text-slate-500 uppercase tracking-widest px-1">Sales Tax (%)</label>
                        <input 
                          type="number"
                          className="w-full bg-slate-950 border border-slate-800 rounded-2xl px-4 py-3 text-sm text-slate-400 font-mono outline-none focus:border-indigo-500"
                  ...
                  <div className="flex flex-col justify-end p-12 bg-indigo-600/10 border border-indigo-500/20 rounded-[40px] shadow-2xl items-baseline gap-6 relative overflow-hidden">
                    ...
                    <div className="w-full space-y-2">
                       <div className="flex justify-between items-center text-slate-500 text-[10px] font-bold uppercase tracking-widest">
                         <span>Subtotal</span>
                         <span>${calculateSubtotal(activeEstimate.lineItems).toLocaleString()}</span>
                       </div>
                       <div className="flex justify-between items-center text-slate-500 text-[10px] font-bold uppercase tracking-widest">
                         <span>Adjustments (Margin + Tax)</span>
                         <span>+ ${(activeEstimate.total - calculateSubtotal(activeEstimate.lineItems)).toLocaleString()}</span>
                       </div>
                       <div className="pt-4 border-t border-slate-800/50 flex justify-between items-baseline">
                         <p className="text-[11px] font-bold text-slate-500 uppercase tracking-widest">Grand Total</p>
              ...
              <div className="w-[450px] bg-slate-950 border-l border-slate-800 p-10 overflow-y-auto animate-in slide-in-from-right z-20 custom-scrollbar shadow-2xl">
                <div className="flex items-center justify-between mb-10">
                  <div className="flex items-center gap-4">
                    <div className="w-10 h-10 bg-indigo-500/10 rounded-xl flex items-center justify-center text-indigo-400 border border-indigo-500/20"><i className="fas fa-bolt-lightning"></i></div>
                    <div>
                      <h3 className="text-xs font-black text-white uppercase tracking-widest">Bid Auditor</h3>
                      <p className="text-[10px] text-slate-500 font-bold uppercase tracking-tight">Real-time Risk Analysis</p>
                    </div>
                  </div>
                ...
                <div className="space-y-10">
                  <div className="bg-slate-900/50 border border-indigo-500/20 rounded-[32px] p-10 text-center relative overflow-hidden">
                    <p className="text-[10px] text-slate-500 uppercase font-black tracking-widest mb-4">Confidence Score</p>
                    <div className="text-7xl font-black text-indigo-400 tracking-tighter">{aiInsight.score}%</div>
                  </div>
                  <div className="space-y-4">
                    <p className="text-[10px] text-slate-500 uppercase font-black tracking-widest px-1">Summary</p>
                    <div className="bg-slate-900/50 p-6 rounded-2xl border border-slate-800 shadow-inner">
                      <p className="text-sm text-slate-300 leading-relaxed italic">"{aiInsight.summary}"</p>
                    </div>
                  </div>
                  <div className="space-y-4">
                    <p className="text-[10px] text-red-500 uppercase font-black tracking-widest px-1">Risks Detected</p>
                    {aiInsight.risks.map((risk, i) => (
                      <div key={i} className="flex gap-4 text-xs text-slate-300 bg-red-500/5 border border-red-500/10 p-5 rounded-2xl">
            ...
            {suggestions && (
              <div className="w-[450px] bg-slate-950 border-l border-slate-800 p-10 overflow-y-auto animate-in slide-in-from-right z-20 custom-scrollbar shadow-2xl">
                <div className="flex items-center justify-between mb-10">
                  <div className="flex items-center gap-4">
                    <div className="w-10 h-10 bg-emerald-500/10 rounded-xl flex items-center justify-center text-emerald-400 border border-emerald-500/20"><i className="fas fa-wand-sparkles"></i></div>
                    <div>
                      <h3 className="text-xs font-black text-white uppercase tracking-widest">Scope Advisor</h3>
                      <p className="text-[10px] text-slate-500 uppercase font-bold tracking-tight">Suggested Scope Expansions</p>
                    </div>
                  </div>
                  ...
                  {suggestions.map((item, i) => (
                    <div key={i} className="bg-slate-900 border border-slate-800 p-6 rounded-3xl hover:border-emerald-500/30 transition-all group shadow-xl">
                      <div className="flex justify-between items-start mb-3">
                        <h4 className="text-sm font-black text-white group-hover:text-emerald-400 transition-colors">{item.name}</h4>
                        <span className="text-[9px] font-mono text-emerald-400 bg-emerald-500/5 px-2 py-0.5 rounded-full border border-emerald-500/10">${item.suggestedRate.toLocaleString()}/unit</span>
                      </div>
            ...
            {marketInsight && (
              <div className="w-[450px] bg-slate-950 border-l border-slate-800 p-10 overflow-y-auto animate-in slide-in-from-right z-20 custom-scrollbar shadow-2xl">
                <div className="flex items-center justify-between mb-10">
                  <div className="flex items-center gap-4">
                    <div className="w-10 h-10 bg-amber-500/10 rounded-xl flex items-center justify-center text-amber-400 border border-amber-500/20"><i className="fas fa-earth-americas"></i></div>
                    <div>
                      <h3 className="text-xs font-black text-white uppercase tracking-widest">Market Intelligence</h3>
                      <p className="text-[10px] text-slate-500 font-bold uppercase tracking-tight">Real-time Grounded Pricing</p>
                    </div>
                  </div>
                ...
                <div className="space-y-8">
                  <div className="prose prose-invert prose-sm">
                    <div className="text-slate-300 leading-relaxed bg-slate-900 border border-slate-800 p-6 rounded-[32px] shadow-inner text-xs">
                      {marketInsight.text}
                    </div>
                  </div>

                  <div className="space-y-4">
                    <h4 className="text-[10px] font-black text-slate-500 uppercase tracking-widest px-1">Verified Sources</h4>
                    <div className="space-y-3">
                      {marketInsight.sources.map((src, i) => (
                        ...
                        >
                          <div className="flex items-center gap-3">
                            <i className="fas fa-link text-slate-600 group-hover:text-amber-500"></i>
                            <span className="text-[10px] font-bold text-slate-400 group-hover:text-white truncate max-w-[250px]">{src.title}</span>
                          </div>
                          <i className="fas fa-external-link-alt text-[10px] text-slate-700"></i>
                        </a>
                      ))}
                    </div>
                  </div>
...
const NavBtn = ({ icon, label, active, onClick }: { icon: string, label: string, active?: boolean, onClick: () => void }) => (
  <button onClick={onClick} className={`w-full flex items-center gap-4 px-4 py-3.5 rounded-2xl transition-all group ${active ? 'bg-indigo-600/10 text-indigo-400 font-black border border-indigo-500/20 shadow-lg shadow-indigo-500/5' : 'text-slate-400 hover:text-white hover:bg-slate-800'}`}>
    <div className={`w-8 h-8 rounded-lg flex items-center justify-center transition-all ${active ? 'bg-indigo-600 text-white' : 'bg-slate-800 group-hover:bg-slate-700'}`}><i className={`fas ${icon} text-xs`}></i></div>
